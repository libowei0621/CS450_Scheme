...
(define (make-new-machine)
    (let ((pc (make-register))
          ...
          ;;local variable about breakpoint
          (labels '())
          (break #t)
          (current-label '*unkonwed*)
          (current-line 0)
          (breakpoint-line 0))
      (let ((the-ops
                ...
                (begin
                  (if (and (not (null? (instruction-label (car insts))))
                      ...
                      (begin
                        ...
                        (set! breakpoint-line (cdr (assoc current-label
                                                          labels)))
                        (set! current-line 0)
                        (set! break #t))
                      '())
                  (set! current-line (+ current-line 1))
                  (if (and (= current-line breakpoint-line) break)
                      (begin 
                        (set! break #f)
                        (display "reach the break point ")
        ...
        (define (cancel-breakpoint label)
          (define (delete-label first-part last-part)
            (cond ((null? last-part)
                   (error "The label is not in the list -- CANCEL-BREAKPOINT"
                          label))
                  ((eq? (caar last-part) label) (append first-part (cdr
                                                                    last-part)))
                  (else (delete-label (cons (car last-part) first-part)
                                      (cdr last-part)))))
         ...
         (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
           (if (symbol? next-inst)
               (if (label-exist? labels next-inst)
                   (error "The label already existed" next-inst)
                   (let ((insts 
                          (if (null? insts)
                              '()
                              (cons (make-instruction-with-label
                                     (instruction-text (car insts)) next-inst)
                                    (cdr insts)))))
                   (receive insts                              ;; either like
                                                               ;; this
                            (cons (make-label-entry next-inst  ;;  (it's a
                                                               ;;  label)
                                                    insts)
                                  labels))))
