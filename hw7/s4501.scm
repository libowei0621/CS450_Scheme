...
;;type-of procedure which will return which type the input expression is.
...
;;special-form procedure which will return true if the input exp is a special
;;form.
...
(define (xeval exp env)
  (let ((type (type-of exp)))
    (let ((action (lookup-action type)))
      (if action
          (action exp env)
          (cond ((special-form? exp)
                 (begin (display "Special form: ")
                        (display exp)
                        (newline)))
                ((thunk? exp) (xeval (cadr exp) (caddr exp)))
                ((reference-obj? exp) (xeval (cadr exp) (caddr exp)))
                ...
                ((application? exp)
                 (let ((proc (xeval (operator exp) env)))
                   (if (primitive-procedure? proc)
                       ...
                       (let ((variable-list (procedure-parameters proc)))
                         (xapply proc (list-of-values variable-list
                                                      (operands exp)
                                                      env))))))
                (else
...
(define (list-of-values list exps env)
  (if (eq? list '())
      (if (no-operands? exps)
          '()
          (cons (xeval (first-operand exps) env)
                (list-of-values '() (rest-operands exps) env)))
      (cond ((delayed? (car list))
             (cons (make-thuck (first-operand exps) env)
                   (list-of-values (cdr list) (rest-operands exps) env)))
            ((reference? (car list))
             (cons (make-referance (first-operand exps) env)
                   (list-of-values (cdr list) (rest-operands exps) env)))
            ((dynamic? (car list))
...
(define (xtend-environment vars vals base-env)
  (if (= (length vars) (length vals))
      (begin (push-frame (make-frame vars vals))
  ...
  (define (env-loop env)
    (define (scan vars vals)
      (cond ((null? vars)
             (env-loop (enclosing-environment env)))
            ((eq? var (car vars))
             (if (reference-obj? (car vals))
                 (set-variable-value! (cadar vals) val (caddar vals))
                 (set-car! vals val)))
            (else (scan (cdr vars) (cdr vals)))))
    (if (eq? env the-empty-environment)
...
;;wrapper procedure for quotation special form.

(define (quoted-action exp env)
  (text-of-quotation exp))

;;wrapper procedure for lambda special form.
...
(define (locally-defined?-action exp env)
  (let ((symbol (cadr exp)))
...
(define (make-unbound!-action exp env)
  (let ((symbol (cadr exp)))
...
(define (locally-make-unbound!-action exp env)
  (let ((symbol (cadr exp)))
...
(install-special-form 'defined? defined?-action)
(install-special-form 'locally-defined? locally-defined?-action)
(install-special-form 'make-unbound! make-unbound!-action)
(install-special-form 'locally-make-unbound! locally-make-unbound!-action)
  ...
  (if (lookup-action name)
      ...
      (if (eq? the-global-environment the-empty-environment)
          (set! the-global-environment
                (xtend-environment (cons name '())
                                   (cons (list 'primitive action) '())
                                   the-empty-environment))
          (let ((frame (first-frame the-global-environment)))
            (define (scan vars vals)
              (cond ((null? vars)
                    ...
                    ((eq? name (car vars))
                     (set-car! vals (list 'primitive action)))
                    (else (scan (cdr vars) (cdr vals)))))
            (scan (frame-variables frame)
                  (frame-values frame))))))
...
;; Procedures which implement the delayed tag
...
(define (make-thunk body env)
  (list 'thunk body env))

(define (remove-tag parameters)
  (if (eq? parameters '())
      '()
      (if (pair? (car parameters))
          (cons (cadar parameters) (remove-tag (cdr parameters)))
          (cons (car parameters) (remove-tag (cdr parameters))))))

;; Procedures which implement the reference tag

(define (reference? exp)
  (tagged-list? exp 'reference))

(define (reference-obj? exp)
  (tagged-list? exp 'reference-obj))

(define (make-referance var enviroment)
  (define (find-env sym)
    (define (env-loop env)
      (define (find? vars)
        (cond ((null? vars) #f)
              ...
              (else (find? (cdr vars)))))
      (if (eq? env the-empty-environment)
          (error "Cannot find the reference")
          (let ((frame (first-frame env)))
             (if (find? (frame-variables))
                env
                (env-loop (enclosing-environment env))))))
    (env-loop enviroment))
  (list 'reference-obj var (find-env var)))

;; Procedures which implement the dynamic tag
...
(define (push-frame frame)
  (set! the-dynamic-environment (cons frame the-dynamic-environment)))
