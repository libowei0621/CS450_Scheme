...
(define (compile-apply exp target linkage)
  (let ((proc (cadr exp))
        (param (caddr exp)))
    (let ((p-code (compile proc 'proc 'next))
          (op-code (compile param 'argl 'next)))
...
(define (map-argl exp) (caddr exp))

(define (compile-map exp target linkage)
  (let ((proc-code (compile (map-proc exp) 'proc 'next))
        (argl-code (compile (reverse (map-argl exp)) 'temp1 'next)))
    (preserving '(env continue)
                proc-code
                (preserving '(proc continue)
                   argl-code
                   (cons-map-result target linkage)))))

(define (cons-map-result target linkage)
  (let ((map-loop (make-label 'map-loop))
        (after-map (make-label 'after-map))
        (consequent-linkage
         (if (eq? linkage 'next) after-map linkage)))
    (let ((map-initial (make-instruction-sequence '() (list target) 
                                                      (list target)
                         `((assign ,target (const ())))))
          (map-test (make-instruction-sequence '(temp1) '(argl) '(argl)
                        `((test (op null?) (reg temp1))
                          (branch (label ,consequent-linkage))
                          (assign argl (op car) (reg temp1)))))
          (map-cons (make-instruction-sequence '(temp1 temp2)
                                               (list 'temp1 target)
                                               (list 'temp1 target)
      ...
      (append-instruction-sequences
           ...
           (append-instruction-sequences 
              map-loop
              map-test
              (compile-procedure-call 'temp2 'next)
              map-cons)
           after-map))))
